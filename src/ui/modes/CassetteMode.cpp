...
namespace {
constexpr int SHELL_X = 24, SHELL_Y = 30, SHELL_W = 272, SHELL_H = 150;
constexpr int HUB_L_X = 96, HUB_R_X = 224, HUB_Y = 120;
constexpr int HUB_R = 13;
constexpr int TAPE_MIN = 16, TAPE_MAX = 40;
}  // namespace
...
void CassetteMode::enter(const AppState &st, const ViewCtx &ctx) {
  ...
  M5.Display.fillRect(SHELL_X + 14, SHELL_Y + 12, SHELL_W - 28, 52,
                      theme::pal.bg);
  M5.Display.drawRect(SHELL_X + 14, SHELL_Y + 12, SHELL_W - 28, 52, edge);
  drawArt(ctx.art_path, SHELL_X + 18, SHELL_Y + 16, 44);

  M5.Display.setFont(theme::fontArtist());
  M5.Display.setTextColor(theme::pal.text, theme::pal.bg);
  char lines[2][WRAP_MAX_LINE];
  const int n = wrapText(st.pb.title, 180, lines, 2);
  for (int i = 0; i < n; ++i) {
    M5.Display.setCursor(SHELL_X + 70, SHELL_Y + 18 + i * 16);
    M5.Display.print(lines[i]);
  }
  M5.Display.setFont(theme::fontSmall());
  M5.Display.setTextColor(theme::pal.dim, theme::pal.bg);
  wrapText(st.pb.artist, 180, lines, 1);
  M5.Display.setCursor(SHELL_X + 70, SHELL_Y + 52);
  M5.Display.print(lines[0]);

  // Window the reels sit behind.
  M5.Display.fillRoundRect(SHELL_X + 40, SHELL_Y + 74, SHELL_W - 80, 62, 4,
                           theme::pal.bg);
  M5.Display.drawRoundRect(SHELL_X + 40, SHELL_Y + 74, SHELL_W - 80, 62, 4, edge);
